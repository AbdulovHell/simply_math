...
namespace Project {
	namespace Core {
		...
		data_list* Init() {
			ProjectError::Init();
			//math_obj* temp;
			data_list* data = new data_list();//TODO: нужно больше умных указателей
			data->in = L"@start#";
			...
			//	temp->close_list();
			//	temp->point_left = new math_obj(L"^", flags::power, flags::nthng, flags::nthng, 0, temp->point_collar, new math_obj(L"/", flags::division, flags::nthng, flags::nthng, 0, new math_obj(L"0", flags::numbr, flags::real, 1), temp->point_collar->point_right, NULL), temp);
