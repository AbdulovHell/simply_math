	...
	namespace Core {
		...
		private:
			...
			double processing(var_const *pointer)
			{
				...
				wstring rd = pointer->read(L"type");
				if (rd == L"oper+")
				...
				//Поэтому у каждой функции должна быть переменная с уникальным указателем
					...
					return processing(pointer->point_collar->point_right);				
			}
